...
export function ServiceCard({ service, index }: ServiceCardProps) {
  const Icon: ComponentType<{ className?: string }> =
    SERVICE_ICONS[service.icon] || Shield;
  ...
  return (
    ...
    >
      <Link
        ...
        className="group bg-card shadow-premium hover:shadow-elevated relative block h-full rounded-2xl p-6 transition-all duration-300"
      >
